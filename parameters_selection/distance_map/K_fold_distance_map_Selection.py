...
# 定义数据集
distance_path = "/home/bli/homology/Alphafold_test/parameters_selection/distance_map/blast_pkl_2716"

for distance in range(6,16):

    #遍历每个distance的文件夹pkl_6 ~ pkl_15
    data_path = os.path.join(distance_path, "pkl_"+ str(distance)+"/train")

    print("...............pkl_" + str(distance) + " train data loading...............")
    ...
    print("...............pkl_" + str(distance) + " data loading completed and 5 kFold train started...............")
    ...
    for train_idx, test_idx in kfold.split(dataset):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if test_accuracy < best_loss:
                ...
                torch.save(model.state_dict(), '/home/bli/homology/Alphafold_test/parameters_selection/distance_map/kFold_' + str(k) + "_best_model.pt")
            # print(f'Epoch: {epoch}, Train_Loss: {train_accuracy:.8f}, Test_Loss: {test_accuracy:.8f}')

            # print('Training finished.')


        model.load_state_dict(torch.load('/home/bli/homology/Alphafold_test/parameters_selection/distance_map/kFold_' + str(k) + "_best_model.pt"))
        model.eval()
    ...
    # 将中值和标准偏差显示出来
    print(f'For distance = {distance}, Median R² = {median_r2:.3f} ± {std_r2:.3f}')
    del model
    torch.cuda.empty_cache()  # 清空GPU缓存（如果在GPU上运行）
