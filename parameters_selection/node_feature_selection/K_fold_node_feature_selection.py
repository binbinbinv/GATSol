...
# 定义数据集
data_path1 = "/home/bli/GNN/Graph_bin/data/homology/alphafold_test/fold_completed_pkl_ESM/train"
data_path2 = "/home/bli/GNN/Graph_bin/data/homology/alphafold_test/fold_completed_pkl_BLOSUM62/train"
data_path3 = "/home/bli/GNN/Graph_bin/data/homology/alphafold_test/fold_completed_pkl_BLOSUM62+ESM/train"

i=0
in_channels_list = [1280,20,1300]

for node_feature_path in [data_path1, data_path2, data_path3]:
    print("...............node_feature_path_" + node_feature_path.replace("/home/bli/GNN/Graph_bin/data/homology/alphafold_test/fold_completed_pkl_","") +" Selection" + " train data loading...............")

    dataset = load_graph_dataset(node_feature_path)
    ...
    print("...............node_feature_path_" + node_feature_path.replace("/home/bli/GNN/Graph_bin/data/homology/alphafold_test/fold_completed_pkl_","") + " data loading completed and 5 kFold train started...............")

    batch_size = 16

    # 存储每个 learning_rate 的 r² 值
    r2_values = []

    # 循环遍历 num_heads
    k = 0   #计数第几折
    r2_per_distance = []   # 存储每个 distance 的五折交叉验证结果
    for train_idx, test_idx in kfold.split(dataset):
        k += 1
        # print("...............pkl_" + str(distance) + " : " + str(k) + "/5 kFold is training...............")
        ...
        for epoch in range(1, epochs + 1):
            ...
            if test_accuracy < best_loss:
                ...
                torch.save(model.state_dict(), '/home/bli/GNN/Graph_bin/data/homology/parameters_selection/node_feature_selection/kFold_' + str(k) + "_best_model.pt")
            # print(f'Epoch: {epoch}, Train_Loss: {train_accuracy:.8f}, Test_Loss: {test_accuracy:.8f}')

            # print('Training finished.')


        model.load_state_dict(torch.load('/home/bli/GNN/Graph_bin/data/homology/parameters_selection/node_feature_selection/kFold_' + str(k) + "_best_model.pt"))
        model.eval()
    ...
    # 将中值和标准偏差显示出来
    print(f'node_feature_selection = {node_feature_path.replace("/home/bli/GNN/Graph_bin/data/homology/alphafold_test/fold_completed_pkl_","")}, Median R² = {median_r2:.3f} ± {std_r2:.3f}')
