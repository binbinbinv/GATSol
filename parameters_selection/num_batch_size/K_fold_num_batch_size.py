...
for num_batch_size in [1,2,4,8,16,32,64,128,256,512]:
    print("...............num_batch_size = " + str(num_batch_size) + " data loading completed and 5 kFold train started...............")
    ...
    for train_idx, test_idx in kfold.split(dataset):
        ...
        model = GATClassifier(in_channels = in_channels, hidden_channels = hidden_channels, num_heads = num_heads, num_layers = num_layers).to(device)
        k += 1
        ...
        for epoch in range(1, epochs + 1):
            ...
            if test_accuracy < best_loss:
                ...
                torch.save(model.state_dict(), '/home/bli/homology/Alphafold_test/parameters_selection/num_batch_size/kFold_' + str(k) + "_best_model.pt")
            # print(f'Epoch: {epoch}, Train_Loss: {train_accuracy:.8f}, Test_Loss: {test_accuracy:.8f}')

            # print('Training finished.')


        model.load_state_dict(torch.load('/home/bli/homology/Alphafold_test/parameters_selection/num_batch_size/kFold_' + str(k) + "_best_model.pt"))
        model.eval()
    ...
    # 将中值和标准偏差显示出来
    print(f'For num_batch_size = {num_batch_size}, Median R² = {median_r2:.3f} ± {std_r2:.3f}')
    # del model
    # torch.cuda.empty_cache()  # 清空GPU缓存（如果在GPU上运行）
