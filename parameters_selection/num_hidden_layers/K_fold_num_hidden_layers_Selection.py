...
for num_hidden_layers in range(1,11):
    
    print("...............num_hidden_layers = " + str(num_hidden_layers) + " data loading completed and 5 kFold train started...............")
    ...
    for train_idx, test_idx in kfold.split(dataset):
        model = GATClassifier(in_channels, hidden_channels, num_heads, num_layers = num_hidden_layers).to(device)
        k += 1
        ...
        for epoch in range(1, epochs + 1):
            ...
            if test_accuracy < best_loss:
                ...
                torch.save(model.state_dict(), '/home/bli/homology/Alphafold_test/parameters_selection/num_hidden_layers/kFold_' + str(k) + "_best_model.pt")
            # print(f'Epoch: {epoch}, Train_Loss: {train_accuracy:.8f}, Test_Loss: {test_accuracy:.8f}')

            # print('Training finished.')


        model.load_state_dict(torch.load('/home/bli/homology/Alphafold_test/parameters_selection/num_hidden_layers/kFold_' + str(k) + "_best_model.pt"))
        model.eval()
    ...
    # 将中值和标准偏差显示出来
    print(f'For num_hidden_layers = {num_hidden_layers}, Median R² = {median_r2:.3f} ± {std_r2:.3f}')
    # del model
    # torch.cuda.empty_cache()  # 清空GPU缓存（如果在GPU上运行）
