...
# 定义数据集
data_path = "/home/bli/homology/dataset/Esol/fold_completed_pkl_BLOSUM62+ESM/train"

for learning_rate in [0.0000005,0.00000005]:
    print("...............Learning_rate_" + "Selection" + " train data loading...............")
    ...
    print("...............learning_rate_" + str(learning_rate) + " data loading completed and 5 kFold train started...............")
    ...
    for train_idx, test_idx in kfold.split(dataset):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if test_accuracy < best_loss:
                ...
                torch.save(model.state_dict(), '/home/bli/homology/Alphafold_test/parameters_selection/learning_rate/kFold_' + str(k) + "_best_model.pt")
            # print(f'Epoch: {epoch}, Train_Loss: {train_accuracy:.8f}, Test_Loss: {test_accuracy:.8f}')

            # print('Training finished.')


        model.load_state_dict(torch.load('/home/bli/homology/Alphafold_test/parameters_selection/learning_rate/kFold_' + str(k) + "_best_model.pt"))
        model.eval()
    ...
    # 将中值和标准偏差显示出来
    print(f'For learning_rate = {learning_rate}, Median R² = {median_r2:.3f} ± {std_r2:.3f}')
    del model
    torch.cuda.empty_cache()  # 清空GPU缓存（如果在GPU上运行）
